...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if name in memory:
                ...
                self.send_header('Location',memeory[name]);
                self.end_headers();
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-trpe','text/plain;charset=utf-8')
            self.end_headers()
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type','text/plain;charset=utf-8')
            self.end_headers()
            self.wfile.write('Could not fetch URI',format(longuri).encode())
